...
def run_experiment(run_name, out_dir='./results', seed=None,
    ...
    #model = models.ConvClassifier(in_size=(3,32,32), out_classes=10, filters=filters, pool_every=pool_every, hidden_dims=hidden_dims)
    ...
    trainer = training.TorchTrainer(model, CrossEntropyLoss, optimizer, device)
    fit_res = trainer.fit(dl_train = dl_train,dl_val = dl_val,num_epochs = epochs, kw = kw, early_stopping = early_stopping)
...
def CrossEntropyLoss(output, y):
    ...
    x = torch.sum(output, 1)
    x = x.reshape(x.shape[-1] , 1)
    output = output/x
    return (-torch.sum(y * torch.log(output), 1)).mean()
