...
class TorchTrainer():
    ...
    def fit(self, dl_train: DataLoader, dl_val: DataLoader,
        ...
        for epoch in range(num_epochs):
            ...
            # ====== YOUR CODE: ======
            if epoch > 0:
                prev_train_l = sum(train_l) / len(train_l)
            train_l, train_a = self.train_epoch(dl_train, **kw)
            train_loss.extend(train_l)
            train_acc.append(train_a)
            test_l, test_a = self.test_epoch(dl_val, **kw)
            val_loss.extend(test_l)
            val_acc.append(test_a)
            if early_stopping is not None and epoch > 0:
                if (sum(train_l) / len(train_l) + 0.002) >= prev_train_l:
                    early_stopping -= 1
                if early_stopping == 0:
                   print("committed early stopping")
                   break   
            # ========================
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        l = []
        for j in y:
            l = [1 if i == j else 0 for i in range(10)]
            ls.append(l)
        y = torch.tensor(ls, dtype=torch.float64)
        #y = F.one_hot(y, num_classes=10)
        #y = torch.tensor(np.array(y.numpy()), dtype = torch.float64)    
        if self.device:
            y = y.to(self.device)             
        ...
        #n_samples += y.shape[0]
        num_correct += sum([1 if y[i][pre[i]] == 1 else 0 for i in range(y.shape[0])]) 
        loss = loss.item()
        # ========================
    ...
    def test_batch(self, batch) -> BatchResult:
        ...
        with torch.no_grad():
            ...
            if self.device:
                y = y.to(self.device)
                outputs = outputs.to(self.device)   
            #y = (F.one_hot(y.float(), num_classes=10)).float()
            loss = self.loss_fn(outputs, y)
            num_correct = 0
            _,pre = torch.max(outputs, 1)
            #n_samples += y.shape[0]
